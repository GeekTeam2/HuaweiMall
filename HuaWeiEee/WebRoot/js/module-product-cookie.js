	...
	function joinCart(){
		if(!$.cookie("goods")){
			...
			for(var i = 0; i < cart_arr.length; i++){
				goodshtml += '<div class = "cart_goods"><i><input type="checkbox" /></i><a href="#"><img src="../' + $("#pro_gallerys").find("li").eq(0).find("img").attr("src") + '" /></a><div class = "cart_goods_detail"><p class = "cart_title"><a href="#">' + $("#goods_title").html() + '</a></p><p class = "cart_color">' + $("#sel_color").html() + '</p><p class = "cart_price"><span class = "cart_price_before">' + $("#pro_price").html() + '</span><span class = "cart_price_now">' + $("#pro_price").html() + '</span><span id = "cart_goods_count">x' + cart_arr[i].num + '</span></p></div></div>';
			...
			var price = $("#pro_price").html().substring(1);
			$("#sum_now").html('¥ ' + parseInt(price) * num + '.00');
			$("#sum_before").html('¥ ' + parseInt(price) * num + '.00');
		}
